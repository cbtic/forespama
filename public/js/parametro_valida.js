...
function datatablenew() {
    var oTable = $('#tblOrdenCompra').dataTable({
        "bServerSide": true,
        "sAjaxSource": "/parametro/listar_total_orden_compra_tienda_ajax",
        "bProcessing": true,
        ...
        "aoColumnDefs": [
            ...
            },
            {
                "mRender": function(data, type, row) {
					...
					if(row.estado_validacion == 1){
						estado_validacion = "Cerrado";
					}
    ...
    $('#tblOrdenCompra tbody').on('click', 'tr', function () {
        ...
        if (anSelected.length != 0) {
			var odtable = $("#tblOrdenCompra").DataTable();
			var idOrdenCompra = odtable.row(this).data().id;
            $("#id_orden_compra").val(idOrdenCompra);
            //alert(idSolicitud);
            ...
            cargarPagoCubicaje(idOrdenCompra);
...
function cargarPagoCubicaje(id){
	...
	//$("#tblCubicaje tbody").html("");
	$("#divValidaParametro").html("");
	
	$.ajax({
			url: "/parametro/cargar_parametro_orden_compra/"+id,
			type: "GET",
			success: function (result) {  
					//$("#tblCubicaje tbody").html(result);
					$("#divValidaParametro").html(result);
