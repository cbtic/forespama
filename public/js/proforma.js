				...
				},
				{
					"mRender": function (data, type, row) {
						...
						if(almacenUsuario.some(almacen => almacen.id_user == row.id_usuario) && row.id_cerrado==1){
							...
							html += '<button style="font-size:12px" type="button" class="btn btn-sm btn-success" data-toggle="modal" onclick="$modalProforma('+row.id+')" ><i class="fa fa-edit"></i> Editar</button>'; 
						}else{
							html += '<button style="font-size:12px" type="button" class="btn btn-sm btn-success" data-toggle="modal" onclick="$modalProforma('+row.id+')" disabled><i class="fa fa-edit"></i> Editar</button>'; 
						}
						if(row.id_cerrado==1){
