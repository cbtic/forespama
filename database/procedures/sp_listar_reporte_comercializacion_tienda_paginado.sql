...
BEGIN
	...
	case when oc.id_canal = 4 then
		case 
		    when om.id_orden_compra is not null then tdoc.cantidad
		    else tdoc.cantidad
		end 
		else ocd.cantidad_requerida 
	end as cantidad,
	case when om.id_orden_compra is not null then
		coalesce(tdoc.cantidad_despacho, 0) 
		else coalesce(ocd.cantidad_despacho, 0) 
	end as cantidad_despacho,
	case when om.id_orden_compra is not null then
		coalesce(tdoc.cantidad - coalesce(tdoc.cantidad_despacho, 0), 0) 
		else coalesce(ocd.cantidad_requerida - coalesce(ocd.cantidad_despacho, 0), 0) 
	end as cantidad_cancelada,
	ocd.cerrado, u."name" vendedor,
	t.denominacion tienda ';
    ...
    v_where := ' WHERE 1=1 and (oc.id_tipo_documento = ''2'' or oc.id_tipo_documento = ''4'') and oc.estado_pedido = ''1'' and ocd.estado = ''1'' and oc.estado = ''1'' ';
