...
begin
	...
	If p_codigo<>'' Then
	 v_where:=v_where||'And ccp.codigo = '''||p_codigo||''' ';
	End If;

	If p_producto<>'' Then
	 v_where:=v_where||'And ccp.id_producto = '''||p_producto||''' ';
	End If;

	If p_estado<>'' Then
	 v_where:=v_where||'And ccp.estado  = '''||p_estado||''' ';
	End If;
	...
	If v_count::Integer > p_limit::Integer then
		v_scad:='SELECT '||v_campos||v_col_count||v_tabla||v_where||' Order By ccp.id desc LIMIT '||p_limit||' OFFSET '||p_pagina||';'; 
	else
		v_scad:='SELECT '||v_campos||v_col_count||v_tabla||v_where||' Order By ccp.id desc;'; 
	End If;
