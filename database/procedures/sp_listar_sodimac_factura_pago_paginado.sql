-- DROP FUNCTION public.sp_listar_sodimac_factura_pago_paginado(varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, refcursor);

CREATE OR REPLACE FUNCTION public.sp_listar_sodimac_factura_pago_paginado(p_fecha_ini character varying, p_fecha_fin character varying, p_tipo character varying, p_serie character varying, p_numero character varying, p_estado_pago character varying, p_observacion_pago character varying, p_dias_pagado character varying, p_color character varying, p_anulado character varying, p_ruc_empresa character varying, p_pagina character varying, p_limit character varying, p_ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
begin
	...
	v_tabla=' (select c.id, c.serie, c.numero, c.tipo, TO_CHAR(c.fecha,''dd-mm-yyyy'') fecha, c.destinatario, c.cod_tributario, c.subtotal, c.impuesto, c.total, c.estado_pago, c.anulado, c.estado_sunat sunat, (select oc.numero_orden_compra_cliente from orden_compras oc inner join orden_compra_detalles ocd on oc.id = ocd.id_orden_compra left join valorizaciones v on ocd.id = v.pk_registro where v.id_comprobante = c.id and oc.estado =''1'' limit 1) numero_orden_compra_cliente, c.moneda, sfd.numero_documento numero_documento_sodimac, sfd.importe_total, sfd.importe_inicial, abs(sfd.importe_retencion) importe_retencion, sfd.importe_detraccion, case when c.anulado = ''S'' then 2 when sfd.importe_total is null then 0 else 1 end as estado_pago_sodimac, CASE when sfd.importe_total is not null and c.total::float = sfd.importe_inicial::float THEN 1 when sfd.importe_total is not null and c.total::float != sfd.importe_inicial::float then 2 else 0 END AS coincide_total_inicial, sf.fecha_pago, DATE_PART(''day'', COALESCE(sf.fecha_pago, CURRENT_DATE) - c.fecha)::int AS dias_diferencia_pago ' ||
			  ...
			  'left join sodimac_facturas sf2 on sfd2.id_sodimac_factura = sf2.id '||
			  'Where 1 = 1 and csh.estado = ''1'' '|| v_filtros_fecha_comprobante_historico || ') union_table ';
