-- DROP FUNCTION public.sp_listar_ingreso_produccion_detalle_paginado(varchar, varchar, varchar, varchar, varchar, varchar, varchar, varchar, refcursor);

CREATE OR REPLACE FUNCTION public.sp_listar_ingreso_produccion_detalle_paginado(p_tipo_documento character varying, p_fecha character varying, p_numero_ingreso_produccion character varying, p_almacen_destino character varying, p_area character varying, p_estado character varying, p_pagina character varying, p_limit character varying, p_ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
begin
	...
	v_campos=' ip.id, tm.denominacion tipo_documento, ip.fecha, ip.codigo numero_ingreso_produccion, ip.estado, a.denominacion almacen_destino, u.name usuario_ingreso, ut.denominacion area, p.codigo, p.denominacion producto, ipd.cantidad ';
