-- DROP FUNCTION public.sp_listar_comprobante_sodimac_paginado(varchar, varchar, varchar, varchar, varchar, varchar, varchar, refcursor);

CREATE OR REPLACE FUNCTION public.sp_listar_comprobante_sodimac_paginado(p_fecha_ini character varying, p_fecha_fin character varying, p_tipo character varying, p_serie character varying, p_numero character varying, p_pagina character varying, p_limit character varying, p_ref refcursor)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
begin
	...
	v_where = ' Where 1 = 1 and  c.id_empresa in (''23'',''187'') and c.anulado != ''S'' ';
