...
begin
	...
	v_tabla=' (select ep.id, ''INGRESO'' tipo, ep.fecha_ingreso fecha_movimiento, tm.denominacion tipo_documento, tm2.denominacion unidad_origen, ' ||
			...
			'and e.estado = ''1'') ' ||
			'end razon_social, ' ||
			'ep.codigo, ep.fecha_comprobante, ep.estado, ep.created_at, ep.cerrado, ''1'' id_tipo, ep.id_tipo_documento, ep.unidad_origen id_unidad_origen, a.denominacion almacen, ep.id_almacen_destino id_almacen, ep.id_proveedor, u.name usuario_recibe ' ||
			'from entrada_productos ep ' ||
			'left join tabla_maestras tm ON ep.id_tipo_documento = tm.codigo::int and tm.tipo = ''48'' ' ||
			...
			'and ep.estado = ''1'' ' ||
			'union all ' ||
			'select sp.id, ''SALIDA'' tipo, sp.fecha_salida fecha_movimiento, tm.denominacion tipo_documento, tm2.denominacion unidad_origen, ' ||
			...
			'and e.estado = ''1'') ' ||
			'end razon_social, ' ||
			'sp.codigo, sp.fecha_comprobante, sp.estado, sp.created_at, sp.cerrado, ''2'' id_tipo, sp.id_tipo_documento, sp.unidad_destino id_unidad_origen, a.denominacion almacen, sp.id_almacen_salida id_almacen, null id_proveedor, u.name usuario_recibe ' ||
