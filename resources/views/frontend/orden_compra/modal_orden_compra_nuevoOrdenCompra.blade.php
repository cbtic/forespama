...
function cargarDetalle(){
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            let marcaBase = '<option value="">--Seleccionar--</option>';
            result.marca.forEach(marca=>{
                marcaBase += `<option value="${marca.id}">${marca.denominiacion}</option>`;
            });

            let productoBase = '<option value="">--Seleccionar--</option>';
            result.producto.forEach(producto=>{
                productoBase += `<option value="${producto.id}">${producto.codigo} - ${producto.denominacion}</option>`;
            });

            let unidadBase = '<option value="">--Seleccionar--</option>';
            result.unidad_medida.forEach(unidad=>{
                unidadBase += `<option value="${unidad.codigo}">${unidad.denominacion}</option>`;
            });
            ...
            result.orden_compra.forEach(orden_compra => {
                ...
                let marcaOptions = marcaBase.replace(`value="${orden_compra.id_marca}"`, `value="${orden_compra.id_marca}" selected`);

                let productoOptions = productoBase.replace(`value="${orden_compra.id_producto}"`, `value="${orden_compra.id_producto}" selected`);

                let unidadMedidaOptions = unidadBase.replace(`value="${orden_compra.id_unidad_medida}"`, `value="${orden_compra.id_unidad_medida}" selected`);
                ...
                <tr>
                    ...
                    <td><input name="cantidad_ingreso[]" id="cantidad_ingreso${n}" class="cantidad_ingreso form-control form-control-sm" value="${orden_compra.cantidad_requerida}" type="text" ${bloqueado ? 'readonly' : ''} oninput="calcularCantidadPendiente(this);calcularSubTotal(this);calcularPrecioUnitario(this);recalcularPorcentajeDescuento(this);calcularDescuentoProducto(this);"></td>
            ...
            $('.select-marca').select2({ width: '100%' });
...
function agregarProducto(){
    ...
    for (var i = 0; i < cantidad; i++) { 
        ...
        var descripcion = '<input name="id_orden_compra_detalle[]" id="id_orden_compra_detalle${n}" class="form-control form-control-sm" value="${orden_compra.id}" type="hidden"><input name="id_autorizacion_detalle[]" id="id_autorizacion_detalle' + n + '" class="form-control form-control-sm" value="2" type="hidden"><select name="descripcion[]" id="descripcion' + n + '" class="form-control form-control-sm" ' +(!tieneRolVendedor ? 'onChange="verificarProductoSeleccionado(this, ' + n + ');validarMuestraExhibicion(this);validarServicio(this);calcularSubTotal(this);calcularPrecioUnitario(this);calcularDescuentoProducto(this);"' : 'onChange="obtenerCodInterno(this, ' + n + ');validarMuestraExhibicion(this);validarServicio(this);calcularSubTotal(this);calcularPrecioUnitario(this);calcularDescuentoProducto(this);"') + '> ' + opcionesDescripcion +' </select>';
        ...
        var cantidad_ingreso = '<input name="cantidad_ingreso[]" id="cantidad_ingreso' + n + '" class="cantidad_ingreso form-control form-control-sm" value="" type="text" oninput="calcularSubTotal(this);calcularPrecioUnitario(this);validarMuestraExhibicion(this);calcularDescuentoProducto(this)">';
