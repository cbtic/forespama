...
.no {padding-right:3px;padding-left:0px;display:block;width:200px;float:left;font-size:14px;text-align:right;padding-top:5px}
.si {padding-right:0px;padding-left:3px;display:block;width:200px;float:left;font-size:14px;text-align:left;padding-top:5px}
...
function fn_save(){
	...
	var tipo = 0;
	if($('#tipo_').is(":checked"))tipo = 1;
	
    $.ajax({
			url: "/empresa/send_usuario_empresa",
            type: "POST",
            data : {_token:_token,id:id,user_id:user_id,ubicacion_id:ubicacion_id,id_proveedor:id_proveedor,tipo:tipo},
...
function cargar_tipo(){
	...
	if(tipo==0)$("#divUbicacion").show();
	if(tipo==1)$("#divProveedor").show();
...
$(document).ready(function() {
	...
	$('#usuario_').keyup(function() {
		this.value = this.value.toLocaleUpperCase();
	});
		
	$('#usuario_').focusin(function() { $('#usuario_').select(); });
	...
	$('#proveedor_').keyup(function() {
		this.value = this.value.toLocaleUpperCase();
	});
		
	$('#proveedor_').focusin(function() { $('#proveedor_').select(); });
	
	$('#proveedor_').autocomplete({
		appendTo: "#proveedor_busqueda",
		source: function(request, response) {
			$.ajax({
			url: '/proveedor/list_proveedor/'+$('#proveedor_').val(),
			dataType: "json",
			success: function(data){
			   var resp = $.map(data,function(obj){
					var hash = {key: obj.id, value: obj.proveedor};
					return hash;
			   }); 
			   response(resp);
			},
