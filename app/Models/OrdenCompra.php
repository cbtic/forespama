...
class OrdenCompra extends Model
{
    ...
    public function listar_reporte_comercializacion_sin_igv_ajax($p){

        return $this->readFuntionPostgres('sp_listar_reporte_comercializacion_sin_igv_paginado',$p);
    ...
    function getDetalleOrdenCompraId($id){
        ...
        ocd.valor_venta_bruto, ocd.precio_venta, ocd.valor_venta, ocd.id_descuento, ocd.descuento, ocd.id_autorizacion 
    ...
    function getDetalleOrdenCompraIdControlProduccion($id){
        ...
        ocd.valor_venta_bruto, ocd.precio_venta, ocd.valor_venta, ocd.id_descuento, ocd.descuento, ocd.comprometido
        from orden_compra_detalles ocd 
        inner join productos p on ocd.id_producto = p.id
        inner join orden_compras oc on ocd.id_orden_compra = oc.id
        left join marcas m on ocd.id_marca=m.id
    ...
    function getDetalleOrdenCompraIdAbierto($id){
        ...
        coalesce((select k.saldos_cantidad from kardex k where id_producto = ocd.id_producto and id_almacen_destino = 3  order by 1 desc limit 1),0)stock_ves, --ves
        coalesce((select k.saldos_cantidad from kardex k where id_producto = ocd.id_producto and id_almacen_destino = 2  order by 1 desc limit 1),0)stock_oxa, --oxa
        ocd.valor_venta_bruto, ocd.precio_venta, ocd.valor_venta, ocd.id_descuento, ocd.descuento
        from orden_compra_detalles ocd 
        inner join productos p on ocd.id_producto = p.id
        inner join orden_compras oc on ocd.id_orden_compra = oc.id
        left join marcas m on ocd.id_marca=m.id
    ...
    function getOrdenCompraByIdPdf($id){
        ...
        where e2.id = oc.id_empresa_compra) end cliente,
        e2.razon_social empresa_vende, to_char(oc.fecha_orden_compra,'dd-mm-yyyy') fecha_orden_compra, 
        oc.numero_orden_compra, tm.denominacion tipo_documento, oc.estado, tm2.denominacion igv, oc.numero_orden_compra_cliente, 
        oc.total, oc.sub_total, oc.igv, COALESCE (oc.descuento, 0 , oc.descuento) descuento,
        case when oc.id_canal = 4 then 
        (select t.direccion || ' - ' || dp.desc_ubigeo || ' - ' || p.desc_ubigeo  || ' - ' || d.desc_ubigeo direccion_completa 
        ...
        (select occe.direccion || ' - ' ||  dp.desc_ubigeo || ' - ' || p.desc_ubigeo || ' - ' || d.desc_ubigeo ||
        case
            when occe.referencia is not null and occe.referencia <> ''
            then ' - ' || occe.referencia
            else ''
        end 
        ...
        and occe.estado = '1'
        order by occe.id desc
        limit 1) end direccion
        from orden_compras oc 
        inner join empresas e2 on oc.id_empresa_vende = e2.id 
        inner join tabla_maestras tm on oc.id_tipo_documento = tm.codigo ::int and tm.tipo = '54'
        inner join tabla_maestras tm2 on oc.igv_compra = tm2.codigo ::int and tm2.tipo = '51'
    ...
    function getOrdenCompraPagoById($id){
        ...
        $cad = "select ocp.id, ocp.fecha, tm.denominacion tipodesembolso, ocp.importe, observacion, nro_guia, nro_factura, nro_cheque, foto_desembolso, ocp.id_orden_compra
    ...
    function getDetalleProductosNoComprometidosId(){
        ...
        and opd.estado ='1'
        and opd.cerrado ='1'
        and opd.id_producto = ocd.id_producto) cantidad_orden_produccion
        from orden_compra_detalles ocd 
    ...
    function getOrdenCompraByIdExcel($id){
        ...
        e2.razon_social empresa_vende, 
        (select e3.ruc from empresas e3 
        where e3.id = oc.id_empresa_compra) ruc_empresa_compra, to_char(oc.fecha_orden_compra,'dd-mm-yyyy') fecha_orden_compra, 
