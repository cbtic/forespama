...
class GuiaInternaDetalle extends Model
{
    ...
    function getDetalleGuiaInternaPdf($id){
        ...
        $cad = "select gid.id, ROW_NUMBER() OVER (PARTITION BY gid.id_guia_interna ) AS row_num, p.denominacion producto, gid.cantidad, tm.denominacion estado_producto, tm2.denominacion unidad_medida, m.denominiacion marca from guia_interna_detalles gid 
