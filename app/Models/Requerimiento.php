...
class Requerimiento extends Model
{
    ...
    function getDetalleRequerimientoId($id){
        ...
        rd.id_estado_producto , rd.cantidad, r.id_almacen_salida, r.id_unidad_origen, r.id_almacen_destino,
        ...
        where oc.id_requerimiento = r.id and ocd.id_producto = rd.id_producto) cantidad_atendida, coalesce(rd.observacion,'') observacion, coalesce(rd.observacion_atencion,'') observacion_atencion, rd.id_usuario_solicita, rd.id_prioridad
    ...
    function getRequerimientoById($id){
        ...
        $cad = "select r.id, tm.denominacion tipo_documento, r.id_tipo_documento, a.denominacion almacen, r.fecha, r.codigo, u.name responsable_atencion, tm2.denominacion estado_atencion, r.sustento_requerimiento, u2.name usuario_solicita, r.observacion, r.aprobado, u3.name usuario_aprueba
        from requerimientos r
        ...
        left join users u3 on r.id_usuario_aprueba = u3.id
        where r.id ='".$id."'";
    ...
    function getControlDetalleRequerimientoId($id){
        ...
        (select coalesce(sum(ocd.cantidad_requerida),0) from orden_compras oc
        inner join orden_compra_detalles ocd on ocd.id_orden_compra = oc.id  and ocd.estado='1'
    ...
    function getDetalleRequerimientoIdAbierto($id){
        ...
        $cad = "select rd.id,  ROW_NUMBER() OVER (PARTITION BY rd.id_requerimiento ) AS row_num, p.numero_serie item, rd.id_producto, p.codigo, p.denominacion nombre_producto, rd.id_marca, rd.id_unidad_medida, 
        rd.id_estado_producto , rd.cantidad, r.id_almacen_destino, r.id_almacen_salida, r.id_unidad_origen,
        (select coalesce(sum(ocd.cantidad_requerida),0) from orden_compras oc
        inner join orden_compra_detalles ocd on ocd.id_orden_compra = oc.id and ocd.estado='1'
        where oc.id_requerimiento = r.id and ocd.id_producto = rd.id_producto and oc.estado ='1') cantidad_atendida,
