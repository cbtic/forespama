...
class AsientoContableVenta extends Model
{
    ...
    function generarAsientosVentas(){
        ...
        end anulado ,'D' debe_haber, c.cod_tributario ruc_cliente, c.destinatario razon_social, /*centro_costos consultar*/ to_char(c.fecha_vencimiento,'yyyy-mm-dd') fecha_vencimiento,
    ...
    function generarAsientosVentasNC(){
        ...
        'CC' tipo_documento, c.serie || c.numero numero_documento, to_char(c.fecha,'yyyy-mm-dd') fecha_documento, (select c2.tipo from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) tipo_documento_referencial, 
        (select c2.serie ||' '|| c2.numero from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) numero_documento_referencial, c.impuesto igv, '' valor_isc, c.impuesto_factor tasa_igv,
        c.total importe, 'VTA' tasa_conversion, ocp.tasa_cambio tc, 'CC' ||' '|| c.serie ||'-'|| c.numero glosa_documento, ocp.glosa_movimiento,
        case 
            when c.anulado = 'N' then false
            else true
        end anulado ,'H' debe_haber, c.cod_tributario ruc_cliente, c.destinatario razon_social, /*centro_costos consultar*/ to_char(c.fecha_vencimiento,'yyyy-mm-dd') fecha_vencimiento,
        (select to_char(c2.fecha,'yyyy-mm-dd') from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) fecha_documento_referencial, false exportacion, '' otros_impuestos, '' exonerado, '' otros_cargos, '' impuesto_bolsa,
        (select c2.numero from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) numero_matriz, (select c2.tipo from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) tipo_matriz,
        (select c2.serie from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) serie_matriz
        from comprobantes c 
    ...
    /*function generarAsientosVentasByNC($serie_matriz, $tipo_matriz, $numero_matriz){
        ...
        end anulado ,'H' debe_haber, c.cod_tributario ruc_cliente, c.destinatario razon_social, to_char(c.fecha_vencimiento,'yyyy-mm-dd') fecha_vencimiento,
        (select to_char(c2.fecha,'yyyy-mm-dd') from comprobantes c2 where c.id_comprobante_ncnd = c2.id ) fecha_documento_referencial, false exportacion, '' otros_impuestos, '' exonerado, '' otros_cargos, '' impuesto_bolsa
        from comprobantes c 
        inner join orden_compras oc on  NULLIF(c.orden_compra,'')::int = oc.id
        left join orden_compra_pagos ocp on oc.id = ocp.id_orden_compra
        ...
        left join starsoft_banco_detalles sbd on sb.id = sbd.id_starsoft_banco and c.id_moneda = sbd.id_moneda 
        where c.serie <> 'E001'
        and c.tipo = '".$tipo_matriz."'
        and c.serie = '".$serie_matriz."'
        and c.numero = '".$numero_matriz."'
        order by c.id desc";
    ...
    function asientosContableVentaDetallePendientes($id){
        ...
        $cad = "select
            coalesce(cc.cuenta_venta, cc.cuenta) cuenta_venta,
            sum(cd.valor_venta_bruto) total_pu, 'H' debe_haber
    ...
    function asientosContableVentaIgvPendientes($id){
        ...
        $cad = "select cd.id, cd.id_comprobante, cd.afect_igv, '4011100' cuenta, 'H' debe_haber
        from comprobante_detalles cd 
    ...
    function asientosContableVentaIgvNCPendientes($serie_matriz, $tipo_matriz, $numero_matriz){

        $cad = "select cd.id, cd.id_comprobante, cd.afect_igv, '4011100' cuenta, 'D' debe_haber
        from comprobante_detalles cd 
        inner join comprobantes c on cd.id_comprobante = c.id 
        where cd.estado ='1'
        and c.serie ='".$serie_matriz."'
        and c.tipo ='".$tipo_matriz."'
        and c.numero ='".$numero_matriz."'
    ...
    function asientosContableVentaDetalleNCPendientes($serie_matriz, $tipo_matriz, $numero_matriz){

        $cad = "select
            coalesce(cc.cuenta_venta, cc.cuenta) cuenta_venta,
            sum(cd.valor_venta_bruto) total_pu, 'D' debe_haber
        from comprobante_detalles cd
        left join comprobantes c on cd.id_comprobante = c.id
        left join productos p on cd.codigo = p.codigo and p.estado = '1'
        left join familia_contables fc on p.id_familia_contable = fc.id
        left join cuenta_contables cc on fc.id_plan_contable = cc.id
        where c.serie ='".$serie_matriz."'
        and c.tipo ='".$tipo_matriz."'
        and c.numero ='".$numero_matriz."'
        group by COALESCE(cc.cuenta_venta, cc.cuenta)";
