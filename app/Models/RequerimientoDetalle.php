...
class RequerimientoDetalle extends Model
{
    function getDetalleRequerimientoPdf($id){
        ...
        $cad = "select rd.id, ROW_NUMBER() OVER (PARTITION BY rd.id_requerimiento order by rd.id asc) AS row_num, p.numero_serie, p.denominacion producto, p.codigo, rd.cantidad, tm.denominacion estado_producto, tm2.denominacion unidad_medida, m.denominiacion marca, rd.observacion 
