...
class RequerimientoDetalle extends Model
{
    function getDetalleRequerimientoPdf($id){
        ...
        $cad = "select rd.id, ROW_NUMBER() OVER (PARTITION BY rd.id_requerimiento order by rd.id asc) AS row_num, p.id id_producto, p.numero_serie, p.denominacion producto, p.codigo, rd.cantidad, tm.denominacion estado_producto, tm2.denominacion unidad_medida, m.denominiacion marca, rd.observacion, 
        (select pi2.ruta_imagen from producto_imagenes pi2 where pi2.id_producto = p.id order by pi2.id asc limit 1) ruta_imagen, p2.apellido_paterno ||' '|| p2.apellido_materno ||' '|| p2.nombres usuario, tm3.denominacion prioridad
