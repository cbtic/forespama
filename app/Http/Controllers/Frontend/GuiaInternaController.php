...
class GuiaInternaController extends Controller
{
    public function __construct(){
		...
		$this->middleware('can:Guia Transportista')->only(['create_transportista']);
	}
    ...
    public function create_transportista(){
		...
		return view('frontend.guia_interna.create_transportista',compact('tipo_documento','transporte_razon_social','empresa','persona'));

	}

    public function listar_guia_transportista_ajax(Request $request){

		$guia_transportista_model = new GuiaTransportista;
        $p[]=$request->fecha;
        $p[]=$request->numero_guia;
        $p[]=$request->numero_documento;
        $p[]=$request->empresa;
		...
		$p[]=$request->NumeroPagina;
		$p[]=$request->NumeroRegistros;
		$data = $guia_transportista_model->listar_guia_transportista_ajax($p);
		$iTotalDisplayRecords = isset($data[0]->totalrows)?$data[0]->totalrows:0;
    ...
    public function modal_guia_transportista($id){
		...
		if($id>0){
            $guia_transportista = GuiaTransportista::find($id);
		}else{
        ...
        $motivo_traslado = $tablaMaestra_model->getMaestroByTipo(63);
        $departamento = $ubigeo_model->getDepartamento();
        $serie_guia = $sede_model->getSerie($id_sede, 'GT');
        $tipo_documento_cliente = $tablaMaestra_model->getMaestroByTipo(75);
        $unidad = $tablaMaestra_model->getMaestroByTipo(43);

        return view('frontend.guia_interna.modal_guia_transportista_nuevoGuiaTransportista',compact('id','guia_transportista','producto','motivo_traslado','departamento','serie_guia','id_user','tipo_documento_cliente','unidad'));

    }

    public function send_guia_transportista(Request $request)
    {
        $id_user = Auth::user()->id;
        $tabla_maestra_model = new TablaMaestra;
        $ubigeo = new Ubigeo;
        ...
        if($request->id == 0){
            ...
            $guia = new Guia;
            $guia_transportista_model = new GuiaTransportista;
            $numero_guia_transportista = $guia_transportista_model->getNumeroGuia($request->serie_guia);
            $numero_guia = $numero_guia_transportista[0]->codigo;
